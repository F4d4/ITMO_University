...
public class MonetizationService {
    ...
    public MonetizationMethodResponse addMonetizationMethod(Long monetizationId,
        ...
        if (request.getType() == MonetizationType.AD) {
            ...
            Long bitrixTaskId = bitrixIntegrationService.createMonetizationReviewTask(
                    method.getId(), request.getAdName(),
                    request.getAdType() != null ? request.getAdType().name() : "");
            if (bitrixTaskId != null) {
                method.setBitrixTaskId(bitrixTaskId);
                method = monetizationMethodRepository.save(method);
            }
