...
# Удаляем функции под номерами 6, 8, 9 (f6, f8, f9)
functions = [f1, f2, f3, f4, f5, f7]
...
def compute_integral(func, a, b, epsilon, method):
    ...
    return result, n,oldN , oldresult
...
if __name__ == "__main__":
    while True:
        ...
        # Проверяем, что выбран корректный номер функции
        if choosen_f not in range(len(functions)):
        ...
        # Если обнаружены точки разрыва, проверяем сходимость
        if len(breakpoints) != 0:
            ...
            if not converges:
                # Если интеграл расходится, выводим сообщение
                print('- Интеграл не существует: интеграл не сходится.')
            else:
                # Если интеграл сходится, вычисляем несобственный интеграл 2-го рода
            ...
            # Если разрывов нет: просто вычисляем интеграл
            epsilon = float(input("Введите требуемую точность вычислений: "))

            for method in methods:
                print(f"\n*   Метод: {method}")
                result, n , oldN , oldresult= compute_integral(func, a, b, epsilon, method)

                if result is not None and n is not None:
                    print(f"Значение интеграла: {result}")
                    print(f"Число разбиений интервала интегрирования для достижения требуемой точности: n = {n}")
                    print(f"Значение интеграла: {oldresult}")
                    print(f"Число разбиений интервала интегрирования для достижения требуемой точности: oldN = {oldN}")
